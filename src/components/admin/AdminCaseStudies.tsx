...
const AdminCaseStudies = () => {
  ...
  const [dialogTab, setDialogTab] = useState<"general" | "content" | "sections" | "services">("general");
  ...
  const openEdit = async (cs: CaseStudy) => {
    ...
    // Fetch sections + linked services
    ...
    <div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editId ? "Edit Case Study" : "Tambah Case Study"}</DialogTitle>
          </DialogHeader>
          ...
          <div className="flex gap-1 mb-4 bg-muted rounded-lg p-1">
            {(["general", "content", "sections", "services"] as const).map((t) => (
              <button
                ...
                className={`flex-1 text-xs font-medium py-2 px-3 rounded-md transition-colors capitalize ${
                  dialogTab === t ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                }`}
              >
          ...
          {dialogTab === "services" && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">Pilih layanan yang digunakan dalam case study ini. Layanan akan ditampilkan di sidebar halaman detail.</p>
