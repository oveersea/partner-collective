      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[
            ...
            <div key={s.label} className="rounded-lg p-2.5 text-center" style={{ background: "hsl(270 30% 20%)" }}>
        ...
        <div className="space-y-2">
          {talentData.map((t) => (
            <div key={t.name} className="flex items-center gap-3 rounded-lg p-2.5" style={{ background: "hsl(270 30% 20%)" }}>
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ background: "hsl(270 40% 30%)" }}>
                {t.name[0]}
              </div>
              ...
              <div className="flex items-center gap-2">
                <div className="w-16 h-1.5 rounded-full overflow-hidden" style={{ background: "hsl(270 30% 25%)" }}>
  ...
  if (activeTab === "hiring") {
    return (
      <div>
        ...
        <div className="space-y-2">
          {hiringData.map((h) => (
            <div key={h.title} className="rounded-lg p-3" style={{ background: "hsl(270 30% 20%)" }}>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          {[
            ...
            <div key={s.label} className="rounded-lg p-3 text-center" style={{ background: "hsl(270 30% 20%)" }}>
        ...
        <div className="flex items-end gap-1.5 h-20 px-2">
          {[40, 65, 50, 80, 60, 90, 75, 85, 70, 95, 80, 88].map((h, i) => (
            <div key={i} className="flex-1 rounded-t" style={{ height: `${h}%`, background: i >= 10 ? "hsl(var(--primary))" : "hsl(270 40% 35%)" }} />
          ))}
        </div>
      ...
      <div className="relative container mx-auto px-6 pt-28 pb-16">
        <div className="grid lg:grid-cols-2 gap-12 items-center min-h-[calc(100vh-7rem)]">
          ...
          <div className="max-w-xl">
            ...
            <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="text-lg leading-relaxed mb-10" style={{ color: "hsl(270 20% 70%)" }}>
            ...
            <motion.form initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} onSubmit={handleCTA} className="flex flex-col sm:flex-row gap-0 mb-4">
              ...
              <Button type="submit" className="h-14 px-8 text-base font-semibold rounded-xl sm:rounded-l-none shrink-0" style={{ background: "hsl(38 90% 55%)", color: "hsl(270 50% 12%)" }}>
                Buat akun gratis
              </Button>
            </motion.form>

            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="text-xs mb-8" style={{ color: "hsl(270 20% 60%)" }}>
          ...
          >
            <div className="rounded-2xl overflow-hidden shadow-2xl border border-white/10" style={{ background: "hsl(270 40% 14%)" }}>
              {/* Browser bar */}
              <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/10" style={{ background: "hsl(270 40% 12%)" }}>
                ...
                <div className="flex-1 mx-4">
                  <div className="h-5 rounded-md px-3 flex items-center text-[10px] text-white/30" style={{ background: "hsl(270 30% 18%)" }}>
              ...
              <div className="flex" style={{ minHeight: 360 }}>
                {/* Sidebar */}
                <div className="w-16 border-r border-white/10 py-3 flex flex-col items-center gap-1" style={{ background: "hsl(270 40% 12%)" }}>
                  {sidebarItems.map((item) => (
                    <button
                      ...
                      style={activeTab === item.id ? { background: "hsl(270 40% 22%)" } : {}}
                    >
