...
import {
  ...
  ArrowLeft, Crown, User, UserPlus, Trash2, MapPin, Globe, Mail, Phone,
...
const VendorDashboard = () => {
  ...
  const fetchVendor = async () => {
    ...
    // Fetch members, documents, credits in parallel
  ...
  const kyc = kycBadge(vendor.kyc_status);
      ...
      <div className="w-full px-4 py-4 md:px-6 md:py-8 pb-28 md:pb-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  {vendor.oveercode && <span className="font-mono">{vendor.oveercode}</span>}
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          <TabsList className="bg-muted p-1 rounded-xl overflow-x-auto flex w-full">
            <TabsTrigger value="overview" className="rounded-lg text-xs md:text-sm"><Building2 className="w-4 h-4 mr-1.5" />Overview</TabsTrigger>
            <TabsTrigger value="members" className="rounded-lg text-xs md:text-sm"><Users className="w-4 h-4 mr-1.5" />Members</TabsTrigger>
            <TabsTrigger value="documents" className="rounded-lg text-xs md:text-sm"><FileText className="w-4 h-4 mr-1.5" />Documents</TabsTrigger>
            <TabsTrigger value="kyc" className="rounded-lg text-xs md:text-sm"><Shield className="w-4 h-4 mr-1.5" />KYC</TabsTrigger>
            ...
            <TabsTrigger value="credits" className="rounded-lg text-xs md:text-sm"><CreditCard className="w-4 h-4 mr-1.5" />Credits</TabsTrigger>
          </TabsList>
              ...
              <div className="bg-card rounded-2xl border border-border p-6 shadow-card space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-foreground">{vendor.industry || "—"}</p>
                  </div>
                  <div>
                    <Label className="text-xs text-muted-foreground">Company Size</Label>
                    <p className="text-foreground">{vendor.company_size || "—"}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    {vendor.website ? (
                      <a href={vendor.website.startsWith("http") ? vendor.website : `https://${vendor.website}`} target="_blank" rel="noopener" className="text-primary hover:underline flex items-center gap-1 text-sm">
              ...
              <div className="bg-card rounded-2xl border border-border p-6 shadow-card space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-foreground flex items-center gap-1"><Mail className="w-3 h-3" /> {vendor.email || "—"}</p>
                  </div>
                  <div>
                    <Label className="text-xs text-muted-foreground">Phone</Label>
                    <p className="text-foreground flex items-center gap-1"><Phone className="w-3 h-3" /> {vendor.phone || "—"}</p>
                  </div>
                  <div>
                    <Label className="text-xs text-muted-foreground">NPWP</Label>
                    <p className="text-foreground font-mono text-xs">{vendor.npwp || "—"}</p>
                  </div>
                  <div>
                    <Label className="text-xs text-muted-foreground">NIB</Label>
                    <p className="text-foreground font-mono text-xs">{vendor.nib || "—"}</p>
                  </div>
                </div>
                {vendor.address && (
                  <div>
          ...
          <TabsContent value="kyc">
            <div className="bg-card rounded-2xl border border-border p-6 shadow-card space-y-4">
              ...
              <div className="flex items-center gap-3 p-4 rounded-xl bg-muted/50">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {vendor.kyc_status === "verified" || vendor.kyc_status === "approved"
              ...
              {(vendor.kyc_status === "unverified" || vendor.kyc_status === "rejected") && isAdmin && (
          ...
          <TabsContent value="projects">
            <div className="bg-card rounded-2xl border border-border p-6 shadow-card">
              ...
              <p className="text-sm text-muted-foreground text-center py-8">No projects yet. Orders assigned to this vendor will appear here.</p>
            </div>
          </TabsContent>
